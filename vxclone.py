...
KANJI = [  # box, 0x56d7, used as placeholder for unidentified Kanji
 ...
 0x91d1, 0x533a, 0x7a7a, 0x718a, 0x6817, 0x3732, 0x90e1, 0x5f62,
...
def serialread(port, count, check = False):
 ...
 if not check:
  ...
  port.write(ACK)
  ack = port.read(1)
  debug('ACK expected: %s, seen: %s' % (repr(ACK), repr(ack)))
 return data
def read(filename):
 if filename is None:
  infile = sys.stdin
...
def serialwrite(port, data, final_block = False):
 ...
 debug('attempting to write %d bytes to %s' % (len(data), port))
 ...
 for index in range(len(data)):
  ...
  readback += echo
  if echo != data[index]:
   debug('echoed data (%s) not same as sent (%s)' % (
    repr(echo), repr(data[index])))
   if final_block:
    print >>sys.stderr, 'quitting at block offset %d' % index
    break
   elif data[index] == '\xff' and echo == '\xff\xff\x06':  # buggy USB adapter
    print >>sys.stderr, 'last character doubled, skipping to next block'
    error = True
    break
 debug('data written: %s' % (snippet(readback)))
 if not final_block and not error:
  ...
  port.write(ACK)
  echo = port.read(port.inWaiting() or 1)
  debug('ACK read back: %s' % repr(echo))
...
def checksum(data, default_offset = -127):
 ...
 for index in range(len(CHECKBYTES)):
  ...
  debug('checksum [0x%x:0x%x] calculated: 0x%x, found: 0x%x' % (
   offset, checkbyte - 1, check, checksum))
  if checksum != check:
   failed = True
   if not filename:
    debug('correcting checksum %d to 0x%x' % (index, check))
...
def clone(action = None, filename = None, port = None):
 ...
 if action == 'read':
  ...
  print >>sys.stderr, 'must specify "read" or "write"'
  sys.exit(1)
def freeband_mod(data, modded):
 ...
 if modbyte == 0xe8:
  debug('image already has mod enabled')
 else:
  debug('enabling mod %02x from %02x' % (0xe8, modbyte))
  modbyte = 0xe8
 if moddedbyte == hardware_setting:
  debug('image hardware byte is already correct')
 else:
  debug('changing modded byte %02x to %02x' % (moddedbyte, hardware_setting))
...
def vxwrite(filename = None, port = None, freeband = False, modded = False):
 ...
 if len(data) != DATASIZE:
  print >>sys.stderr, 'incorrect data length: %d' % len(data)
  sys.exit(1)
